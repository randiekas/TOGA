 ...
 */
class Pertanyaan extends REST_Controller {
    ...
    {
        // Users from a data store e.g. database
        $response["results"] = $this->db->query("select * from v_asks")->result();
        $this->set_response($response, REST_Controller::HTTP_OK); // OK (200) being the HTTP response code
    ...
    public function list_jawaban_post()
    {
        ...
        $response["results"] = $this->db->query("select * from v_ask_comments")->result();
        $this->set_response($response, REST_Controller::HTTP_OK); // OK (200) being the HTTP response code
    }
